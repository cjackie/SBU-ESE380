...
reset:
	...
	ldi r25, 0x00					;variable for keeping the state
	sts state, r25					;store the state
 ...
 * 00: nothing, idle
 * 01: interrupt adc has finish
 * 02: need to start a conversion
...
main:
	...
	cpi r25, 0x04					;check if it's a valid state
...
idle:
	rcall get_queue
	brts task_available
	ldi r16, 0x00					;no task, go to idle state
	sts state, r16
	clt
	ret
task_available:
...
;conversion finished. convert data for display
;data will be stored in SRAM, result_val
adc_finish:
	in r16, ADCL
	...
	ldi r25, 0x03					;
	sts state, r25					;transit to state 03 to display the result
	ret
...
start_adc:
	...
	ldi r25, 0x00
	sts state, r25					;go back to idle state
	ret

;display the value in result_val
;two decimal number. for example of result_val is 145.
;it will be 1.45 on the screen
display:
	lds r16, result_val				;get display value
	push r16					;store value
	ldi ZL, low(adc_msg*2)				;load const string poiter
	ldi ZH, high(adc_msg*2)				;
	rcall load_msg					;store msg to buffer
	...
	rcall load_msg					;store msg to buffer
	ldi ZL, low(line3_msg*2)			;load const string poiter
	ldi ZH, high(line3_msg*2)			;
	rcall load_msg					;store msg to buffer
	pop r16						;
...
;adc connversion complete interrupt
adc_int:
	push r16					;save r16
	in r16, SREG					;save state of sreg	
	push r16
	...
	ldi r16, 0x01					;schedule the task
	rcall add_queue					;add it
...
;*******************
;NAME:      init_time_scheduler:
;FUNCTION:  start the timer 1 to count X second. X is determined
;		by the value passed by r17:r16*0.00256. The compa interrupt
;		will be enabled when counts reach the compared value.
;ASSUME:    nothing
;RETURNS:   nothing
...
;subroutine to conig the PA4 for adc. One time mode, and interrupt enabled
init_adc:
...
;get r17%r16, r17 and r16 are unsigned number
;result is store back in r16
;use 
modulo:
	cp r17, r16
	brlo mod_result
	...
	rjmp modulo
mod_result:
	mov r16, r17					;r17 is the remainder
	ret
