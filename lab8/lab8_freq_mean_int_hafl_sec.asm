/*
 * lab8_freq_mea_int_hafl.asm: a program to measure the 
 *	frequency of the unknown square wave on PortA0.
 *	Timer 1 interrupt is deployed to get accurate 
 *  half second. Then, the frequency is multiplied by 2
 *  to get real frequency, which will be display on 
 *	the LCD display. The frequency is acceptable in
 *  range of 10Hz to 10KHz. Otherwise, "out of range" 
 *  will be displayed.
 *
 ...
 *			Z,Y
 *
 *  Created: 10/30/2014 10:01:09 PM
...
reset:
	...
	cbi DDRA, 0					;set PortD6 as input of the wave
...
timer_done:
	...
	mov r16, r21					;get the count
	mov r17, r22					;high byte
	add r16, r21					;mul the r17:r16 by 2 to get freq
	adc r17, r22					;
	rcall unpack_hex_val_to_bcd			;unpack to bcd
...
;*******************
;NAME:      start_tc1
;FUNCTION:  start the timer 1 to count half second. compare A interrupt 
;			will be generated when half second elapse.
;ASSUME:    nothing
;RETURNS:   nothing
...
start_tc1:								
	...
	out TCNT1L, r16					;
	ldi r16, 0xa1					;prepare to set comparator
	ldi r17, 0x07					;to get half sec
...
line3_message: .db 3, " Gate = 1/2sec  ", 0  ; not used
