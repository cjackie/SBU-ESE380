...
captured_status:
	...
	;this way of compare might have a problem if noise is strong. 0 can't never be
	;achieved
	andi r17, 0x0f					;mask out not used bits
	or r17, r16						;
	cpi r17, 0x00					;check if r17:r16 is 0, namely if voltage is 0
	breq start_capt					;0 voltage, need to start capture
...
start_capture:
	...
	ldi r17, 0
	ldi r16, 10
	rcall less_than					;check if r17:r16 is 0, namely if voltage is 0
	brtc start_capture_ready		;not 0 voltage, ready to capture
	ldi r16, 0x02					;transit to display
	sts state, r16					;
	ret
start_capture_ready:
	...
	ldi r18, 0x04					;data is ready to be processed
	sts state, r18					;
	ret
...
conversion_finish:
	...
	st Z+, r1						;
	st Z, r0						;
	rcall make_beep					;make the beep sound
	ldi r16, 0x00
	sts capture_status, r16			;update capture status. it's captured!
	ldi r16, 0x02					;
	sts state, r16
	ret
